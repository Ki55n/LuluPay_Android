...
class RemittanceDetails : AppCompatActivity() {
 ...
 override fun onCreate(savedInstanceState: Bundle?) {
  ...
  private fun saveRecipient(
    ...
    private suspend fun doesPhoneExist(phoneNumber: String): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                val luluPayDB = LuluPayDB(this@RemittanceDetails)
                val remittanceList: List<RemittanceHistory> = luluPayDB.getAllData()
                remittanceList.any { it.phoneNo == phoneNumber }
            } catch (e: Exception) {
